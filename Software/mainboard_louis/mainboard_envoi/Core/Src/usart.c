...
extern uint8_t tarvos_TX_Tampon[TarvosTxTamponSize];
...
extern int GPSbufferoldpos;
extern int GPSbuffernewpos;
extern int TarvosRXbufferoldpos;
extern int TarvosRXbuffernewpos;
/* USER CODE END 0 */
...
void HAL_UART_MspDeInit(UART_HandleTypeDef* uartHandle)
{
  ...
  /* USER CODE END USART1_MspDeInit 0 */
    ...
    PB7     ------> USART1_RX
    */
    HAL_GPIO_DeInit(GPIOB, USART1_TX_GPS_Pin|USART1_RX_GPS_Pin);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	if(huart->Instance==USART1){
		GPSbufferoldpos = GPSbuffernewpos; //keep track of the last position in the buffer
			...
			for (int i=0;i<500;i++){
			  	  if (GPS_RX_Buffer[i]==36 & GPS_RX_Buffer[i+3]==71 & GPS_RX_Buffer[i+4]==71){
			  		  for (int j=0;j<75;j++){
			  			  GPGGA[j]=GPS_RX_Buffer[i+j];
			  		  }
			  	  }
			    }
