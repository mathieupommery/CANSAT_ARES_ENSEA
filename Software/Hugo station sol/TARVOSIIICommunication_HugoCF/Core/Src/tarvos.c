...
/* --- Functions --- */
void INIT_PERM_TARVOS(uint8_t Hadr, uint8_t Sadr) {		//TarvosIII initialisation
...
void SET_tcMODE(char mode) {		//Changement de mode (transparent,command)
...
uint8_t Get_CRC8(uint8_t * bufP, uint16_t len) {		//Calcule du CRC
...
int SEND_DATA_NETW(uint8_t *data, uint8_t channel, uint8_t dest_adress, int length) {		//To send data

	uint8_t trame_reception[5];
    uint8_t trame[HEAD + length + CRC];			// Longueur totale de la trame : en-tête (5 octets) + données + CRC avec +1 pour CRC
    ...
    trame[5 + length] = Get_CRC8(trame, 5 + length);		    // Calcul du CRC (sur tout sauf le CRC lui-même)

    HAL_UART_Transmit(&hlpuart1, trame, sizeof(trame), 500);		    // Transmission de la trame
...
void SET_Destadr(uint8_t Destadrr){
	...
	uint8_t buffer[5]={0x02,0x08,0x01,Destadrr,0x00};			//on change l'adresse de destination à Destadrr
